...
void usage(){
	printf("Usage:\n");
	printf("\tclient --server <server ip [or hostname]> --user <username>\
		--password <password> --exec <program to execute>\n");   
...
/* 
*	Function  : ClientConnection - Creates a TCP connection with SERVER 
				       and sends the command it.
*	Parameter : char server_ip[MAXLEN]
*		    char username[MAXLEN]
*		    char password[MAXLEN]
*		    char command[MAXLEN]
*	Return	  : NULL
*/
int ClientConnection(char server_ip[MAXLEN],char username[MAXLEN],
			char password[MAXLEN],char command[MAXLEN]){
	...
	/* Check if valid IP address -- Start */
	  if(inet_pton(AF_INET, server_ip, &(serverSocketAddress.sin_addr)) <= 0){
		...
		if(serverIP == NULL){
			...
			bcopy((char *)serverIP->h_addr, (char *) &serverSocketAddress.sin_addr.s_addr,
				 serverIP->h_length);
		}
	}
	...
	if (connect(serverConnection,(struct sockaddr *) &serverSocketAddress,
	...
	while(1){
		...
		if(strstr(recvBuff,"***The End***") != NULL){
			...
			/* Weird way but using backspace reduces string manipulation, 
			though not a scalable way but for current scenario this 
			solution seems to be a fitting one */
...
/* 
*	Function  : main - Initiate client, reads the argument, establishes TCP connection with server 
...
int main(int argc, char** argv){
	...
	ClientConnection(argument.server,argument.username,
			argument.password,argument.command);
